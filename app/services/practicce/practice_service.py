...
class PracticeService:
    def __init__(self, repository: PracticeRepository,apiRepository: PracticeApiRepository):
        # 何もしない
    ...
    async def ai_registration(self, user_id :str, request: ConversationSetCreateRequest) -> ConversationResponse:
        ...
        # AIによるメッセージ一覧の取得
        valueObject = await self.apiRepository.get_generated_conversation(request.user_phrase)
    ...
    async def get_conversations(self, user_id: str) -> ConversationsResponse:
        ...
        entities: List[ConversationEntity]= await self.dbRepository.get_conversations(user_id)
    ...
    async def get_conversation(self, conversation_id: UUID, user_id: str) -> ConversationResponse:
        """会話セットのメッセージ一覧を取得する"""
        entities: List[MessageResponse] = await self.dbRepository.get_conversation(conversation_id, user_id)
        messages = [
            MessageResponse(
    ...
    async def post_test_results(self, user_id: str, request: RecallTestRequest) -> MessageTestResultSummary:
        ...
        try:
            ...
            conversation = await self.dbRepository.get_conversation(request.conversation_id, user_id)

            # 前回のテスト結果を取得
            last_test_result = await self.dbRepository.get_latest_test_result(request.conversation_id)
