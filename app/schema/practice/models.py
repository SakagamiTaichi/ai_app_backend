...
class ConversationModel(Base):
    ...
    test_scores = relationship(
        "ConversationTestScoreModel", back_populates="conversation"
    )
...
class MessageModel(Base):
    ...
    conversation_id = Column(
        UUID(as_uuid=True), ForeignKey("en_conversations.id"), primary_key=True
    )
...
class MessageTestScoreModel(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            [
                "en_conversation_test_scores.conversation_id",
                "en_conversation_test_scores.test_number",
            ],
        ),
    )
